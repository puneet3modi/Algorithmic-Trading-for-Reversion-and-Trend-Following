...
def main() -> None:
    logger = setup_logger("run_backtest_reversion")
    ...
    in_path = f"{processed_dir}/{symbol}_{interval}_reversion_positions.csv"
    if not os.path.exists(in_path):
        raise FileNotFoundError(f"Missing: {in_path}. Run compute_positions_reversion.py first.")

    df = pd.read_csv(in_path)
    df["open_time"] = pd.to_datetime(df["open_time"], utc=True)
    df = df.sort_values("open_time").set_index("open_time")
    df = df.rename(columns={"position_reversion": "position"})
    ...
    out_path = f"{processed_dir}/{symbol}_{interval}_backtest_reversion.csv"
    bt.to_csv(out_path)
